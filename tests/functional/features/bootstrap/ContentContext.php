...
class ContentContext extends BehatContext
{
    ...
    public function createAndPreview($module, $preview, TableNode $table)
    {
        ...
        if ('content' === $preview) {
            ...
            $this->getMinkContext()->clickLink('three-column');
    ...
    protected function fillCurrentEditForm()
    {
        ...
        switch ($module) {
            ...
            case 'block':
                ...
                break;
            default:
                throw new UnexpectedValueException("Unknown module: {$module}");
        }
    }
    ...
    protected function fillItemEditForm($type)
    {
        ...
        switch ($type) {
            ...
            case 'article':
                ...
                $intro = $page->findField('parts[0][intro]')->getValue();
                if (!$intro) {
                    $intro = sprintf('%s-v0-intro', $name);
                }

                $body = $page->findField('parts[0][body]')->getValue();
                if (!$body) {
    ...
    protected function autoFillItemPublishForm(array $fields = [])
    {
        ...
        $this->autoSelectOption('templateId', 'three-column', $fields);
            ...
            switch ($position) {
                ...
                case 'rightColumn':
                    ...
                    $add = $this->getMinkContext()->getElement(
                        ...
                        sprintf(
                            ...
                            strtolower(StaticFilter::execute($position, CamelCaseToDash::class))
                        )
                    );
                ...
                case 'lowerPromo':
                    ...
                    $this->getMinkContext()->selectOption(
                        sprintf('freeBlocks[%s][id]', $position),
                        $value
                    );
    ...
    public function previewFromPublishingWorkflow($name)
    {
        ...
        if (preg_match($regex, $text)) {
            ...
            $this->getMinkContext()->clickLink('with three-column template');
        }
    ...
    protected function followWorkflowItemLink($workflow, $link, $name)
    {
        ...
        $this->getMinkContext()->dumpResponse();
    }
    ...
    public function shuffleBlockSequence($blockName, $direction, $sequenceName, $contentName)
    {
        ...
        $icon = $this->getMinkContext()->getElement(
            ...
            sprintf(
                ...
                strtolower(StaticFilter::execute($sequenceName, CamelCaseToDash::class)),
                $blockName,
                $direction
            )
        );
    ...
    public function assertInTopLevelMenu($name)
    {
        ...
        $locator = sprintf('//*[contains(@class, "main-nav")]//a[contains(., "%s")]', $name);
    ...
    public function assertInSubMenu($name, $of)
    {
        ...
        $locator = sprintf('//*[contains(@class, "sub-nav")]//a[contains(., "%s")]', $name);
        $this->getMinkContext()->getElement('xpath', $locator)->click();
        ...
        $locator = sprintf('//*[@id="breadcrumbs"]//a[contains(., "%s")]', $of);
        $this->getMinkContext()->getElement('xpath', $locator)->click();
    ...
    public function assertPositionInBlockSequence($blockName, $position, $compareBlockName, $sequenceName, $contentName)
    {
        ...
        $this->getMinkContext()->assertPositionInCollectionOfElements(
            'css',
            sprintf('aside.%s-block', strtolower(StaticFilter::execute($sequenceName, CamelCaseToDash::class))),
            $blockName,
            $position,
            $compareBlockName
        );
